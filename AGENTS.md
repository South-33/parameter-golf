...
- Research direction -> multiple independent research passes converged on rotation/incoherence-based quantization stabilization as the strongest new idea, with scale-reparameterization second, but the first export-only blockwise Hadamard probe gave only sub-threshold improvement on `9/3 @ 1024` and a slight regression on `9/3 @ 896` -> move the next serious branch to scale-reparameterization rather than escalating fixed rotations.
